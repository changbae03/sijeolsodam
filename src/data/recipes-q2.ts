...
export const recipesQ2: Recipe[] = [
  ...
  },
  {
    ...
    masterclass: {
      chefIntro:
        '6월의 햇감자는 한 해 중 가장 깨끗한 맛을 냅니다. 캐낸 지 얼마 안 돼 수분이 넉넉하고, 전분은 아직 자리를 잡기 전이라 향이 또렷하죠. 폼므 안나는 이 맛을 가릴 만한 재료를 일절 쓰지 않아요. 버터와 소금, 그게 전부입니다. 그래서 저장 감자보다 지금 같은 햇감자를 쓸 때 그 차이가 훨씬 분명하게 드러나요. 손이 많이 가는 요리예요. 그래도 한 번쯔음 "감자 하나로 이런 걸 만들 수 있구나" 싶은 경험을 드리고 싶어 골랐습니다.',
      ingredientSelection:
        '왁시 품종 — 전분은 적고 수분이 많은 감자 — 가 이 요리엔 제격이에요. 한국 마트에서 자주 보이는 수미감자는 전분질이 좀 강한 편이라 평소엔 부서지기 쉬운데, 햇감자 철에는 그 전분이 아직 덜 잡혀서 오히려 다루기 좋아요. 크기를 맞춰 고르세요. 비슷한 크기끼리 모아야 슬라이스 두께도 따라서 맞춰지거든요. 표면에 푸른빛이 도는 건 싹이 트려는 신호니 피하시고요.',
      miseEnPlace:
        '버터는 미리 약불에 녹여 정제해두세요. 만돌린은 안전 가드까지 한 번 점검하고, 감자 무게보다 무거운 냄비나 내열 접시를 미리 곁에 둡니다. 오븐은 190도로 예열을 시작해두세요. 슬라이스를 시작한 순간부터는 멈출 수 없는 시간과의 싸움이에요. 손 닿는 자리에 모든 걸 미리 꺼내놓는 일, 그게 이 요리의 절반입니다.',
      cookware: {
    ...
    steps: [
      {
        title: '균일하게 슬라이스',
        description:
          '감자는 껍질을 벗기고 만돌린이나 칼로 1.5~2mm로 가능한 균일하게 슬라이스합니다. 재료가 단순할수록 기술이 곧 맛이에요. 두께가 들쭉날쭉하면 익는 속도도 따라 달라지고, 뒤집은 순간 한쪽은 타고 한쪽은 설익은 결과를 마주하게 돼요.',
      ...
      },
      {
        title: '오븐에서 마무리',
        description:
          '190도로 예열한 오븐에 옮겨 30분에서 40분 더 굽습니다. 팬 가장자리에서 버터 끓는 소리가 잦아지고 옆면이 짙은 황금빛으로 물들 즈음이면, 거의 다 온 셈이에요.',
        timerSeconds: 2100,
